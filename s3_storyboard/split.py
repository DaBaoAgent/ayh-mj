...
4. 结尾镜头（2-3秒）：温馨收尾+产品
5. **口播文字量约束（重要）**：每镜 narration ≤ 20 字，全文口播合计 ≤ 100 字
   （视频总时长 10-20 秒 × 快节奏语速 5 字/秒 → 最多 100 字；文案超标会导致配音远长于画面）
6. 每个镜头的 scene_prompt 是给 AI 视频生成模型的提示词：
   - 中文，≤150字
   - 结构：场景(地点/光线) + 主体人物动作 + 产品外观状态 + 镜头运动
   - 要求写实风格、自然光
   - 人物：老年人（60-70岁），穿着得体
   - 产品外观必须与参考图一致（白底图会作为参考图传入）
   - **画面文字约束（重要）**：写法固定为「产品品牌标识保持原样，画面其他位置无任何文字、字幕、水印、贴纸」——
     不要写"无文字水印"这种笼统说法（会导致模型随机生成乱码假字：实战实测）
